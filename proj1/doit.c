...
void print_stats(double wall, struct timeval user, struct timeval sys, long inv, long vol, long maj, long min){
    ...
    cout << setw(40) << "User CPU time: " << (user.tv_usec / 1000) + (user.tv_sec * 1000) << "ms" << endl;
    cout << setw(40) << "System CPU time: " << (sys.tv_usec / 1000) + (sys.tv_sec * 1000) << "ms" << endl;
...
int main(int argc, char **argv) {
    ...
    char **list_output = (char **) calloc(40, sizeof(char));
